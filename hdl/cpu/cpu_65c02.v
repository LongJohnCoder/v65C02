...
always @(posedge clk)
    adj_bcd <= #1 adc_sbc & D;     // '1' when doing a BCD instruction
...
always @(posedge clk)
    if( write_register & RDY )
        AXYS[regsel] <= #1 (state == JSR0) ? DIMUX : AO;
    ...
    if( reset )
        ...
        DECODE  : 
            casex ( IR )
                ...
                8'b0000_0000:   state <= #1 BRK0;
                8'b0010_0000:   state <= #1 JSR0;
                8'b0010_1100:   state <= #1 ABS0;  // BIT abs
                8'b1001_1100:   state <= #1 ABS0;  // STZ abs
                8'b000x_1100:   state <= #1 ABS0;  // TSB/TRB
                8'b0100_0000:   state <= #1 RTI0;  // 
                8'b0100_1100:   state <= #1 JMP0;
                8'b0110_0000:   state <= #1 RTS0;
                8'b0110_1100:   state <= #1 JMPI0;
                8'b0111_1100:   state <= #1 JMPIX0;
`ifdef IMPLEMENT_NOPS             
                8'bxxxx_xx11:   state <= #1 REG;   // (NOP1: 3/7/B/F column)
                8'bxxx0_0010:   state <= #1 FETCH; // (NOP2: 2 column, 4 column handled correctly below)
                8'bx1x1_1100:   state <= #1 ABS0;  // (NOP3: C column)
`endif
                8'b0x00_1000:   state <= #1 PUSH0;
                8'b0x10_1000:   state <= #1 PULL0;
                8'b0xx1_1000:   state <= #1 REG;   // CLC, SEC, CLI, SEI 
                8'b11x0_00x0:   state <= #1 FETCH; // IMM
                8'b1x10_00x0:   state <= #1 FETCH; // IMM
                8'b1xx0_1100:   state <= #1 ABS0;  // X/Y abs
                8'b1xxx_1000:   state <= #1 REG;   // DEY, TYA, ... 
                8'bxxx0_0001:   state <= #1 INDX0;
                8'bxxx1_0010:   state <= #1 IND0;  // (ZP) odd 2 column
                8'b000x_0100:   state <= #1 ZP0;   // TSB/TRB
                8'bxxx0_01xx:   state <= #1 ZP0;
                8'bxxx0_1001:   state <= #1 FETCH; // IMM
                8'bxxx0_1101:   state <= #1 ABS0;  // even D column
                8'bxxx0_1110:   state <= #1 ABS0;  // even E column
                8'bxxx1_0000:   state <= #1 BRA0;  // odd 0 column (Branches)
                8'b1000_0000:   state <= #1 BRA0;  // BRA
                8'bxxx1_0001:   state <= #1 INDY0; // odd 1 column
                8'bxxx1_01xx:   state <= #1 ZPX0;  // odd 4,5,6,7 columns
                8'bxxx1_1001:   state <= #1 ABSX0; // odd 9 column
                8'bx011_1100:   state <= #1 ABSX0; // C column BIT (3C), LDY (BC)
                8'bxxx1_11x1:   state <= #1 ABSX0; // odd D, F columns
                8'bxxx1_111x:   state <= #1 ABSX0; // odd E, F columns
                8'bx101_1010:   state <= #1 PUSH0; // PHX/PHY
                8'bx111_1010:   state <= #1 PULL0; // PLX/PLY
                8'bx0xx_1010:   state <= #1 REG;   // <shift> A, TXA, ...  NOP
                8'bxxx0_1010:   state <= #1 REG;   // <shift> A, TXA, ...  NOP
            endcase

        ZP0     : state <= #1 write_back ? READ : FETCH;

        ZPX0    : state <= #1 ZPX1;
        ZPX1    : state <= #1 write_back ? READ : FETCH;

        ABS0    : state <= #1 ABS1;
        ABS1    : state <= #1 write_back ? READ : FETCH;

        ABSX0   : state <= #1 ABSX1;
        ABSX1   : state <= #1 (CO | store | write_back) ? ABSX2 : FETCH;
        ABSX2   : state <= #1 write_back ? READ : FETCH;

        JMPIX0  : state <= #1 JMPIX1;
        JMPIX1  : state <= #1 CO ? JMPIX2 : JMP0;
        JMPIX2  : state <= #1 JMP0;
        ...
        INDY0   : state <= #1 INDY1;
        INDY1   : state <= #1 INDY2;
        INDY2   : state <= #1 (CO | store) ? INDY3 : FETCH;
        INDY3   : state <= #1 FETCH;

        READ    : state <= #1 WRITE;
        WRITE   : state <= #1 FETCH;
        FETCH   : state <= #1 DECODE;

        REG     : state <= #1 DECODE;
        
        PUSH0   : state <= #1 PUSH1;
        PUSH1   : state <= #1 DECODE;

        PULL0   : state <= #1 PULL1;
        PULL1   : state <= #1 PULL2; 
        PULL2   : state <= #1 DECODE;
        ...
        BRA0    : state <= #1 cond_true ? BRA1 : DECODE;
        BRA1    : state <= #1 (CO ^ backwards) ? BRA2 : DECODE;
        BRA2    : state <= #1 DECODE;
     ...
     if( state == DECODE && RDY )
        casex( IR )
                8'b001x_x100:   // BIT zp/abs/zpx/absx (update N,V,Z)
                                {bit_ins, bit_ins_nv}  <= #1 2'b11;

                8'b1000_1001:   // BIT imm (update Z)
                                {bit_ins, bit_ins_nv}  <= #1 2'b10;

                default:        // not a BIT instruction
                                {bit_ins, bit_ins_nv}  <= #1 2'b00;
        endcase
